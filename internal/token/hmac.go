...
// CookiePrefix — начало имени куки с токеном.
//
// Полное имя включает идентификатор опроса: vote_token_{poll_id}.
//
// Кука ОБЯЗАНА быть своя у каждого опроса, и это не аккуратность, а
// корректность. С единственной кукой на все опросы минт для соседнего опроса
// затирал бы токен текущего, и переход «проголосовал в A → открыл B → вернулся
// в A» выдавал бы в A новую личность вместе с правом голоса. Обход того же
// класса, что и обход перезагрузкой, только через навигацию: технических
// навыков не требует вовсе, а в вечер с несколькими роликами повторяется сам
// собой (architecture.md §5.3).
const CookiePrefix = "vote_token_"

// CookieName — имя куки для конкретного опроса.
func CookieName(pollID uuid.UUID) string { return CookiePrefix + pollID.String() }
